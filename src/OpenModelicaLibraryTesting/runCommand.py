"""
This file is part of OpenModelica.

Copyright (c) 1998-2024, Open Source Modelica Consortium (OSMC),
c/o Linköpings universitet, Department of Computer and Information Science,
SE-58183 Linköping, Sweden.

All rights reserved.

THIS PROGRAM IS PROVIDED UNDER THE TERMS OF GPL VERSION 3 LICENSE OR
THIS OSMC PUBLIC LICENSE (OSMC-PL) VERSION 1.2.
ANY USE, REPRODUCTION OR DISTRIBUTION OF THIS PROGRAM CONSTITUTES
RECIPIENT'S ACCEPTANCE OF THE OSMC PUBLIC LICENSE OR THE GPL VERSION 3,
ACCORDING TO RECIPIENTS CHOICE.

The OpenModelica software and the Open Source Modelica
Consortium (OSMC) Public License (OSMC-PL) are obtained
from OSMC, either from the above address,
from the URLs: http://www.ida.liu.se/projects/OpenModelica or
http://www.openmodelica.org, and in the OpenModelica distribution.
GNU version 3 is obtained from: http://www.gnu.org/copyleft/gpl.html.

This program is distributed WITHOUT ANY WARRANTY; without
even the implied warranty of  MERCHANTABILITY or FITNESS
FOR A PARTICULAR PURPOSE, EXCEPT AS EXPRESSLY SET FORTH
IN THE BY RECIPIENT SELECTED SUBSIDIARY LICENSE CONDITIONS OF OSMC-PL.

See the full OSMC Public License conditions for more details.
"""

import glob
import os
import subprocess
import threading

from rmtree import rmtree

class TimeoutError(Exception):
  pass

class CommandExecutionError(Exception):
  pass

def runCommand(cmd: str, prefix: str, timeout: int, clean: bool, verbose: bool):
  """
  Execute a command in a subprocess with specified parameters.

  Parameters:
  - cmd (str): The command to execute in the subprocess.
  - prefix (str): Prefix to use for temporary files and directories created during execution.
  - timeout (int): Maximum time in seconds to wait for the command to complete.
  - clean (bool): If True, attempt to clean up temporary files and directories after execution.
  - verbose (bool): If True, print verbose output during execution.

  Raises:
  - TimeoutError: If the command execution exceeds the specified timeout.
  - CommandExecutionError: If an error occurs during command execution.

  Example:
  ```
  # Execute a command with a timeout of 10 seconds
  runCommand("ls -l", "/tmp", timeout=10, clean=True, verbose=True)
  ```
  """

  isWin = os.name == 'nt'
  process = [None]
  def target():
    with open(os.devnull, 'w')  as FNULL:
      if isWin:
        process[0] = subprocess.Popen(cmd, shell=True, stdin=FNULL, stdout=FNULL, stderr=FNULL)
      else:
        process[0] = subprocess.Popen(cmd, shell=True, stdin=FNULL, stdout=FNULL, stderr=FNULL, preexec_fn=os.setpgrp)

      while process[0].poll() is None:
        if verbose:
          print("process running... pid: " + str(process[0].pid) + " timeout: " + str(timeout) + " cmd: " + cmd.split('>',1)[0])
        process[0].communicate(1)
        process[0].wait(1)

  thread = threading.Thread(target=target)
  thread.start()
  thread.join(timeout)

  if thread.is_alive():
    process.terminate()
    thread.join(min(10, timeout))
    if thread.is_alive():
      process.kill()
      thread.join()

    raise TimeoutError("Command execution timed out")

  if clean:
    print("---> try clean")
    try:
      lines = open("%s.tmpfiles" % prefix).readlines()
    except:
      lines = []
    for suffix in [".so", ".mos", "", ".o", ".h", ".c", ".cpp", "_info.json", ".xml", ".tmpfiles", ".pipe", ".tmpfiles", ".libs", ".log"]:
      for f in glob.glob(prefix+suffix):
        lines.append(f)
      for f in glob.glob("OM"+prefix+suffix):
        lines.append(f)
    for line in lines:
      f = line.strip()
      if os.path.isdir(f):
        rmtree(f)
      elif os.path.exists(f):
        os.unlink(f)
    try:
      rmtree(prefix)
    except OSError:
      pass

  if process.returncode != 0:
    raise CommandExecutionError("Command execution failed with return code: " + str(process.returncode))

